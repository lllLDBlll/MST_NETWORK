...
//Add Leonardo
void importar_grafo_dot(const char *filename){
	FILE *file;
	...
	int tree[100];//raiz[0], folha[1], peso[2];

	if (filename == NULL){
		fprintf(stderr, "importar_grafo_dot: ponteiros invalidos\n");
		exit(EXIT_FAILURE);
	}
	...
	while (i != -1){
		...
		if(c <= 57 && c >= 48){
			...
			tree[i] = -1;
			if(c <= 57 && c >= 48){
				tree[i-1] = tree[i-1] + c - 48;
 			} 			
		}
		if(c == '}'){
			for(i = 0; tree[i] != -1; i=i+3){
				printf("tree r: %d\n",tree[i]);
				printf("tree f: %d\n",tree[i+1]);
				printf("tree p: %d\n",tree[i+2]);
			}
			for(i = 0; tree[i] != -1; i=i+3){
				//Adicionando as raizes ou vertices
				if(tree[i] == tree[i+3]){
				}else{
					printf("tree r: %d\n",tree[i]);
					vertice = grafo_adicionar_vertice(grafo, tree[i]);
			...
			for(i = 0; tree[i] != -1; i=i+3){
				...
				if(tree[i] == tree[i+3]){
					...
					sucessor = procura_vertice(grafo, tree[i+1]);
					aresta = cria_aresta(vertice, sucessor, tree[i+2]);
					adiciona_aresta(vertice, aresta);
				}
			}
			i = -1;
	...
	fclose(file);
	exportar_grafo_dot("grafo_output.dot", grafo);
}